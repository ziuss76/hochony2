...
function Detail(props) {
  ...
  // find() 는 array 뒤에 위치, 안에 콜백 함수 필요
  // 상품은 array 안의 데이터 하나하나, return 오른쪽엔 조건식,
  // 즉 shoes 안의 id라는 영구번호 = url의 /:id 자리 숫자인지 찾아라!
  // -> 쓰는 이유, 상품의 순서가 변경되었을때도 id고유번호로 상품을 찾아 띄움
  ...
  return (
    <div className="container">
      ...
      <CSSTransition in={스위치} classNames="pop" timeout={500}>
...
function TabComponent(props) {
  ...
  if (props.누른탭 === 0) {
    return <div class="detail-content">저녁8시이전 주문시 새벽배송!</div>;
  } else if (props.누른탭 === 1) {
    return <div class="detail-content">한번 간택 당하면 빠꾸 그런거 없습니다ㅅㄱ</div>;
  } else if (props.누른탭 === 2) {
    return <div class="detail-content">어마어마한 앙큼boy네요 <br/> 저 요염한 빵댕이 뚜들기러 가봐야돼서 그럼이만</div>;
  }
}
